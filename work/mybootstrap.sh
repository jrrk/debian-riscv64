...
#Copy the emulator
sudo cp ../riscv-qemu/riscv64-linux-user/qemu-riscv64 work/debian-riscv64-chroot/usr/bin/qemu-riscv64-static
#Perform the first stage bootstrap
sudo debootstrap --arch=riscv64 --variant=minbase /usr/share/keyrings/debian-ports-archive-keyring.gpg \
     --include=gnupg sid work/debian-riscv64-chroot \
     http://deb.debian.org/debian-ports || (echo Errors occurred. Perhaps your RISCV emulator or operating system is too old ??)
...
sudo chroot work/debian-riscv64-chroot apt install -y \
        ...
        rdate iperf3 dosfstools wamerican file libmagic1 libmagic-mgc gnupg \
